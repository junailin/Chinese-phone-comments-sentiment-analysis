...
global_x = 'z5x'
f = pd.read_csv("data/%s/result.csv" % global_x,sep='\t',header=0)

s = "\n".join(list(f['comment'])) #读取为一个字符串
...
#保存结果并输出
pd.DataFrame(pd.concat(t[1:])).to_csv('data/%s/new_word.txt' % global_x, header = False)
